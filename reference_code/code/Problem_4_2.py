...
from Problem_4_1 import clusters
# %% 1. 数据准备
# 导入文件
...
MODEL.addConstrs(sum(x_3[i, k2, j3] for j3 in J) == sum(x_2[i, k1, j2, k2] for k1 in K for j2 in J) for i in I for k2 in K)
MODEL.addConstrs(sum(x_1[i, j, k1] for i in I for k1 in K) + sum(x_2[i, k1, j, k2] for i in I for k1 in K for k2 in K) + sum(x_3[i, k2, j] for i in I for k2 in K) == 1 for j in J)
                        ...
                        if x_2[i, k1, j2, k2].x:
                            for j3 in J:
                                if  x_3[i, k2, j3].x:
                                    ...
                                    path4 = graph[alpha[i]].shortest_paths(j2, k2, show=False)[0][0][:-1]
                                    path4[0] = path4[0] + "(作业点 2)"
                                    path5 = graph[alpha[i]].shortest_paths(k2, j3, show=False)[0][0]
                                    path5[0] = path5[0] + "(补水点 2)"
                                    path5[-1] = path5[-1] + "(作业点 3)"
                                    path6 = graph[alpha[i]].shortest_paths(j3, beta[i], show=False)[0][0][:-1]
